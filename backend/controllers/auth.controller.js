  ...
  try {
    ...
    try {
      const user = await userModel.create(
        [{ name, email, password : await bcrypt.hash(password,10), phone, address, pincode, otp : hashedOtp,  }],
        {
          session,
        }
      );
...
export const userDetails = async (req, res) => {
  const id = req.userId;
  const data = await userModel.findById(id).select("name email phone address pincode role isAdmin");
